...
let do_file ~verbose ~fuzz path input =
  ...
  let nparsed =
    List.fold_left
      (fun npats (_, _, ps) -> npats + List.length ps)
      0 rules
  in
  let rules =
  ...
  let rnames = List.map (fun r -> r.name) rules |> setify in
...
let () =
  ...
  do_file ~verbose ~fuzz input_path input;
